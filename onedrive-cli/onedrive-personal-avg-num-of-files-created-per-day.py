...
# Calculate the average number of files created per day for each month
for year_month, files in files_by_month.items():
    year, month = year_month
    num_days = calendar.monthrange(year, month)[1]
    num_files = len(files)
    avg_files_per_day = num_files / num_days
    print(f"Month {month} of {year}: {avg_files_per_day:.2f} files/day")